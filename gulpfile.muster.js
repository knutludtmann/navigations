...
gulp.task('pl-copy-min:img', function () {
  return gulp.src('**/*.*', { cwd: resolvePath(paths().source.images) })
    .pipe(imagemin())
    .pipe(gulp.dest(resolvePath(paths().public.images)));
});
...
gulp.task('pl-sass', function () {
  return gulp.src('source/css/**/*.scss')
    ...
    .pipe(sass.sync({ includePaths: ['node_modules/susy/sass'], outputStyle: 'compressed' }).on('error', sass.logError))
...
function watch() {
  ...
  gulp.watch(path.resolve(paths().source.css, '**/*.scss')).on('change', gulp.series('pl-sass', reload, reloadCSS));
  var patternWatches = [
